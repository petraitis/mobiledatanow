/**	$Id: Config.java,v 1.3 2002/06/27 00:19:08 jonc Exp $
 *
 * The Config class allows easy access to a composite set of properties which
 * may be used to configure an application. Config extends Properties so the
 * usual getProperty () etc. methods are available.
 *
 * Once constructed a Config object will contain the set of system properties
 * (as per System.getProperties ()) and the union of all properties from any
 * loaded configuration files.
 *
 * Because all the properties are accumulated into one set you must be careful
 * to avoid name collision. It is advisable to use structured property names
 * using package.class style of notation.
 *
 * The configuration files can be nested, in that any property name that begins
 * with the constant wsl.configfile. will have its name parsed as a URL and read
 * as an additional property file. If the protocol is "resource" (i.e.
 * resource://relative/path/filename) then the resource loader will be used to
 * locate the data relative to the classpath. Any resources/URLs that fail to be
 * loaded will be logged as errors.
 *
 * The root configuration file (s) are obtained from the system properties
 * (any property beginning wsl.configfile.) and from a resource named
 * wsl/config/default.conf loaded from the classpath. The properties defining
 * the additional config files may be defined using the java comand line option
 * -Dproperty.name=value.
 *
 * After construction addConfig may be called to add extra urls or property
 * sets to the Config.
 *
 * The addContext () function can also be called to add (if they exist) the
 * properties in the named context. Contexts are defined in the system or
 * global.conf properties by setting a property of the form:
 * wsl.configcontext.<context name>=<config url>
 *
 * Typical usage in an application would be:
 * // add this app's context to the Config singleton
 * Config.getSingleton ().addContext ("context_name_for_this_app");
 * // start getting properties
 * String aValue = Config.getProp ("a_porperty_key");
 *
 */
package wsl.fw.util;
	...
	extends Properties
{
	...
	public static final ResId
		DEBUG_PROPERTIES	= new ResId ("Config.debug.Properties"),
		DEBUG_ADDING		= new ResId ("Config.debug.Edding"),
		ERR_RESOURCE1		= new ResId ("Config.error.Resource1"),
		ERR_RESOURCE2		= new ResId ("Config.error.Resource2"),
		ERR_ADDING			= new ResId ("Config.error.Adding"),
		WARNING_CONTEXT		= new ResId ("Config.warning.Context");

	// Constants
	public final static String RESOURCE_PREFIX = "resource://";
	public final static String DEFAULT_CONFIG_FILE = RESOURCE_PREFIX
		+ "wsl/config/default.conf";
	...
	getSingleton ()
	{
		// create new singleton if required
		if (s_singleton == null)
			{
	 ...
	 boolean useSystemProperties)
	{
		...
		// init it without using the defaul conf file
		s_singleton.init (useSystemProperties, false);

		// load the specified conf file
	...
	init (
	 boolean useSystemProperties,
	 boolean useDefaultConfigFile)
	{
		// recursively add the properties and nested config from the default
		// config file
		if (useDefaultConfigFile)
			addConfig (DEFAULT_CONFIG_FILE);

		// recursively add the system properties and nested config files.
		// do this second so system properties can override the default config.
	...
	getSubkeys (
	 ...
	 boolean full)
	{
		// the prefix does not have the trailing dot, so add it
		if (keyPrefix != null)
			keyPrefix += CKfw.DOT;

		// create a list to hold the subkeys
		...
		synchronized (this)
		{
			...
			while (nameEnum.hasMoreElements ())
			{
				...
				// check if it matches our prefix (or null prefix)
				if (keyPrefix == null || key.startsWith (keyPrefix))
				{
					// get the sub key part
					String part = key.substring (keyPrefix.length ());

					// check if leaf and add
					if (!leaf || (part.indexOf (CKfw.DOT) == -1))
						subkeys.add ((full) ? key : part);
				}
			}
		}

		// return the subkeys as an arroy of strings
	 ...
	 Properties props)
	{
		...
		synchronized (this)
		{
			while (nameEnum.hasMoreElements ())
			{
				...
				// if it is a nested config file entry then add it recursively
				if (propName.startsWith (CKfw.CONFIG_NAME_PREFIX))
					addConfig (propValue);
			}

			// after all this has been done, include any doLast entries
			while (lastEnum.hasMoreElements ())
			{
				...
				// if it is a nested doLast entry then add it recursively.
				// these dolast entries to not error if the file cannot be found
				// as they are intended for overrides of the basic conf files
				if (propName.startsWith (CKfw.DOLAST_CONFIG_NAME_PREFIX))
					addConfig (propValue, false);
			}
		}

		// having just added some config stuff re-init the log to ensure it
		// has set the correct log level and log stream
		Log.init ();
	}
	 ...
	 boolean logIfMissing)
	{
		...
		{
			URL url = null;
			// first check if it is a resource URL and create a normal URL or
			// a URL for the named resource
			if (configURL.startsWith (RESOURCE_PREFIX))
			{
				// extract the resource name and use it to load a system
				...
				{
					if (logIfMissing)
						Log.error (ERR_RESOURCE1.getText () + " [" + resourceName
							+ "] " + ERR_RESOURCE2.getText ());
					return;
				}
			}
			else
			...
			// create new properties and load from the stream
	 ...
	 String context)
	{
		...
		if (contextUrl != null)
			addConfig (contextUrl);
		else
			Log.warning ("Config.addContext: contex [" + CKfw.CONTEXT_PREFIX
				+ context + "] " + WARNING_CONTEXT.getText ());
	...
	{
		// get the context argument, default if it doesn't exist
		String context = Util.getArg (args, "-context", defaultContext);

		// if there is a context then load it
