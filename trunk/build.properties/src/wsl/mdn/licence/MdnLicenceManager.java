...
public class MdnLicenceManager
{
    ...
    public final static HelpId HID_LICENCE = new HelpId("mdn.licence.MdnLicenceManager");

    public final static String EULA_FILE = "EULA.txt";

    /**
     * Product code constants
     */
     public static int PROD_CODE_MDN_SERVER =
     (
        /*Prod ID*/(0x01 << 16) |
        /*Major version*/(0x01 << 8 ) |
        /*Minor version*/ (0x00)
     );
    ...
    public static boolean showLicenceManager()
    {
        ...
        // set the help manager if none set
        /*if (HelpManager.get() == null)
            try
            {
         ...
         */
        // should not need LocalServerFactory, DataManager or GuiManager
        ...
        try
        {
            ...
            if (lKey == null)
                lKey = makeLicenceKey(PROD_CODE_MDN_SERVER); // new installation
            else if (!isValidLicenceKey(lKey))
            {
    ...
    {
        // set the ResourceManager (must be first as everything uses resource strings)
        ...
        //If MdnLicenceManager is called with a command line argument,
        //Take the argument as the email address to register with
        //(Assuming console-only operation
    ...
    public static void consoleRegistration(String emailAddress, String reinstallRef) {
    	try {
    		...
    		//JDialog dlg = new JDialog(); //Empty dialog
    		//RegisterAppPanel panel = new RegisterAppPanel(dlg, licenceKey, activationKey);
    ...
    public static boolean showEula()
    {
        try
        {
            ...
            {
                TEXT_EULA_ACCEPT.getText(),
                TEXT_EULA_DECLINE.getText()
            };
            ...
            int rv = JOptionPane.showOptionDialog(null, scrollPane,
                TEXT_EULA_TITLE.getText(), JOptionPane.YES_NO_OPTION,
                JOptionPane.PLAIN_MESSAGE, null,
                optionButtons, optionButtons[0]);

            // option zero is accept
            return (rv == 0);
        }
        catch (Exception e)
        {
    ...
    public static void showActivationPanel(LicenceKey lKey, ActivationKey aKey)
    {
        ...
        //dlg.getContentPane().add(helpButton, BorderLayout.SOUTH);
    ...
    public static ActivationKey getActivationKey(LicenceKey lKey) throws Exception
    {
        MdnStore store = getStore();
        if (store != null)
            return store.getActivationKey(lKey);
    ...
    public static LicenceKey getLicenceKey(int productID) throws Exception
    {
        MdnStore store = getStore();
        if (store != null)
            return store.getLicenceKey(productID);
    ...
    public static String getRegisterdEmailAddress() throws Exception
    {
        MdnStore store = getStore();
        if (store != null)
            return store.getRegisteredEmailAddress();
    ...
    public static int getAvailablePublicMessages() throws Exception
    {
        MdnStore store = getStore();
        if (store != null)
            return store.getAvailablePublicMessages();
    ...
    public static int getInstallationReferenceNumber() throws Exception
    {
        MdnStore store = getStore();
        if (store != null)
            return store.getInstallationReferenceNumber();
    ...
    public static LicenceKey makeLicenceKey(int productID) throws Exception
    {
        ...
        //System.out.println("Path for making license key: [" + path + "]["+ addr + "]["+ host + "]");
