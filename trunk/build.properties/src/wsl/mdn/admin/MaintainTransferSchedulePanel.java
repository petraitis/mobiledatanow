 ...
 *
 */
public class MaintainTransferSchedulePanel
    extends WslButtonPanel
    implements ChangeListener, ActionListener
{
    // version tag
    private final static String _ident = "$Date: 2002/06/11 23:35:35 $  $Revision: 1.1.1.1 $ "
        + "$Archive: /Mobile Data Now/Source/wsl/mdn/admin/MaintainTransferSchedulePanel.java $ ";
    ...
    public final static HelpId HID_MAINT_TRANSFERSCHEDULE = new HelpId("mdn.admin.MaintainTransferSchedulePanel");
    ...
    public MaintainTransferSchedulePanel()
    {
        ...
        /**
         * @todo add the data change listeners.
         */
    ...
    {
        // create MonthCalendarTabPane and schedulingsTabPane
        ...
        // add the tab panels for Month Calendar Panel and Schedulings Panel
    ...
    public void actionPerformed(ActionEvent ev)
    {
        try
        {
            ...
            if(ev.getSource().equals(_btnClose))
                closePanel();
            else if (ev.getSource().equals(_cmbDataTransfers))
                dataTransfersComboSelected();
        }
        catch (Exception e)
        {
    ...
    public void stateChanged(ChangeEvent e)
    {
        ...
        if (tabPane == _monthCalendarTabPane)
        {
            ...
            for (int i =0; i < monthCalendarTabPaneButtons.length; i++)
                addButton(monthCalendarTabPaneButtons[i]);
        ...
        else if (tabPane == _schedulingsTabPane)
        {
            ...
            for (int i =0; i < schedulingsTabPaneButtons.length; i++)
                addButton(schedulingsTabPaneButtons[i]);
            addHelpButton(BUTTON_HELP.getText(), HID_MAINT_TRANSFERSCHEDULE);
            addButton(_btnClose);
        }
    }
    ...
    {
        int dataTransferId;
        if (_cmbDataTransfers.getSelectedItem().equals(COMBO_ITEM_ALL.getText()))
            dataTransferId = Type.NULL_INTEGER;
        else
            dataTransferId = ((DataTransfer)(_cmbDataTransfers.getSelectedItem())).getId();
    ...
    {
        if (_cmbDataTransfers.getSelectedItem() == null || _cmbDataTransfers.getSelectedItem().equals(COMBO_ITEM_ALL.getText())
            || ((DataTransfer)(_cmbDataTransfers.getSelectedItem())).getId() != dataTransferId)
        {
            _cmbDataTransfers.selectItem(COMBO_ITEM_ALL.getText());
            return;
        }
