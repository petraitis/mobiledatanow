 ...
 * {@link IParamView} class is stored in a XML file. The XML file wsmapping.xml
 * has a structure like this:<br/><br/>
 * 
 * &lt;mapping datatype="boolean"&gt;<br/>
 *  &lt;visualclass>WSboolean&lt;/visualclass&gt;<br/>
 ...
 *  &lt;defaultvalidatorclass>BooleanValidator&lt;/defaultvalidatorclass&gt;<br/>
 * &lt;/mapping&gt;<br/>
 * <br/>
 * 
 */
public class TypeMapper {
  ...
  //private static final Logger LOGGER = Logger.getLogger(WSMapper.class);
  
  private static TypeMapper m_VCMapper=null;
   ...
   * 
   * @throws MdnException if XML parsing failed.
   */
  private TypeMapper() throws MdnException {
    ...
    try {
      ...
      /*
       * Enables XML schema validation that validates the wsmapper.xml against wsschema.xsd.
       */
      builder.setProperty("http://apache.org/xml/properties/schema/external-schemaLocation","http://www.mobiledatanow.com/WSSchema wsschema.xsd");
      File file=WebServiceFileHelper.getFile("wsschema.xsd",false);
      file=WebServiceFileHelper.getFile("wsmapping.xml",false);
      doc=builder.build(file);
    }
    catch(IOException e) {
      throw new MdnException("error.IOException"+ "error.WSMappingError",e);
    }
    catch(JDOMException e) {
      throw new MdnException("error.JDOMException"+"error.WSMappingError",e);
    }
   ...
   * 
   * @return {@link TypeMapper} instance.
   * @throws MdnException if XML parsing failed.
   */
  public static TypeMapper getInstance() throws MdnException {
    if(m_VCMapper==null)
      m_VCMapper=new TypeMapper();
    return m_VCMapper;
  }
	
  /**
   * Maps a given list of {@link ParamListItem} to a list of Visual Components.
   * This method constructs Visual Components for a Web Service request. The values in the {@link ParamListItem}
   * objects are set to the default. After changing the values, the list of {@link ParamListItem}
   * can be sent to the Web Service.
   * 
   * @param parent parent component.
   * @param list list of {@link ParamListItem} objects.
   * @throws MdnException if {@link IParamView} classes couldn't generated dynamically.
   * @see ParamListItem
   */
	public List<IParamView> mapVCRequest(List<ParamListItem> list, IParamView parent) throws MdnException {
    List<IParamView> addedVC=new ArrayList<IParamView>();
    if(list.size()==0) {
      String labelInfo = "Method requires no arguments.";
    }
    IParamView vc=null;
    for(ParamListItem item:list) {
      if(item.isContainer()) {
        ...
        try {
          ...
          Constructor constructor=clazz.getConstructor(new Class[]{ParamListItem.class});
          vc=(IParamView)constructor.newInstance(new Object[]{item});
        }
        catch(SecurityException e) {
          throw new MdnException("error.SecurityException",e);
        }
        ...
        addedVC.addAll(mapVCRequest(item.getList(), vc));
        ...
        try {
          ...
          Constructor constructor=clazz1.getConstructor(new Class[]{List.class,ParamListItem.class,Class.class});
          vc=(IParamView)constructor.newInstance(new Object[]{addedVC,item,clazz2});
          //Add first array element by default.
          
          ((WSArray)vc).addArrayElement();
        }
        catch(SecurityException e) {
          throw new MdnException("error.SecurityException",e);
        }
        ...
        try {
          ...
          Constructor constructor=classVC.getConstructor(new Class[]{ParamListItem.class});
          vc=(IParamView)constructor.newInstance(new Object[]{item});
   ...
   * {@link ParamListItem} objects that contain the effective data that has to be shown
   * in the Visual Components. For example a {@link ParamListItem} object may contain
   * the integer 5 that now has to be shown in text box.
   * 
   * @param parent parent component.
   * @param list list of {@link ParamListItem}ParamListItem{@link ParamListItem} objects.
   * @return list of {@link IParamView} objects.
   * @throws MdnException if {@link IParamView} classes couldn't generated dynamically.
   */
  /*public List<IVC> mapVCResponse(Composite parent,List<ParamListItem> list) throws MdnException {
    ...
    for(ParamListItem item:list) {
      if(item.isContainer()) {
        ...
        try {
          ...
          throw new MdnException(Properties.getMessage("error.SecurityException"),e);
        }
        catch(IllegalArgumentException e) {
          throw new MdnException(Properties.getMessage("error.IllegalArgumentException"),e);
        }
        catch(NoSuchMethodException e) {
          throw new MdnException(Properties.getMessage("error.NoSuchMethodException"),e);
        }
        catch(InstantiationException e) {
          throw new MdnException(Properties.getMessage("error.InstantiationException"),e);
        }
        catch(IllegalAccessException e) {
          throw new MdnException(Properties.getMessage("error.IllegalAccessException"),e);
        }
        catch(InvocationTargetException e) {
          throw new MdnException(Properties.getMessage("error.InvocationTargetException"),e);
        }
      }
      else if(item.isArray()) {
        IVC vcarray=null;
   ...
   */
  public Map<String,String> getMapping(String javatype) throws MdnException {
    boolean found=false;
