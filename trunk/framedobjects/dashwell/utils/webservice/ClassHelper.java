...
public final class ClassHelper {
  ...
  public String[] getParamNames(Class cl, String methodName, String[] paramTypes)
    ...
    try {
      ...
      return paramNames;
      //throw new MdnException(Config.getProp("error.IOException")
      //    + Config.getProp("error.noSuchFileInWSJarfile"), e);
    }
  ...
  public List<ParamListItem> getParamListFromFields(
    Class[] arguments, String[] labels, Object[] values) throws MdnException{
    ...
    for (int i = 0; i < values.length; ++i){//for (int i = 0; i < arguments.length; ++i){
  ...
  private List<ParamListItem> resultParamListItems;// = new ArrayList<ParamListItem>();
  ...
  public ParamListItem getParamListItemFromFieldsOld(
      Class cl, String label, Object value) throws MdnException{
    boolean isArray = cl.isArray();
    // set cl to its componenttype if cl is an array
    if (isArray){
      cl = cl.getComponentType();
    }
     //TODO make array result
 ...
 //_________________________________________________________________________________
  ...
  //public ParamListItem getParamListItemFromFields(Class cl, String label, Object value) throws MdnException{
	  public ParamListItem getParamListItemFromFields(Class cl, String label, Object value) throws MdnException{
	    ...
	    } else if (mapping.get(VCMapper.ELEMENTNAME_ITEMTYPE).equals(VCMapper.ITEMTYPE_CONTAINER)){	    // container item type
	      System.out.println(">>>>> REGISTERDE OBJECT JAVA TYPE >>>> ");
	      String parentlable = "";
	      Vector<Class> types = new Vector<Class>();
	      Vector<String> labels = new Vector<String>();
	      Vector<Object> values = new Vector<Object>();
	      //-------------------loop on value array object start
		            	  ...
		            	  try {
		    				...
		    				for(int i = 0 ; i<valueMethods.length; i++){
		    					...
		    					if(methodName.equalsIgnoreCase("get"+fieldName)){
		    			        	...
		    			        	//Object fieldValue = setArgValue(field.getType().getSimpleName(), fValue);
		    			        	//System.out.println(">>>> fieldType = <" + field.getType().getSimpleName() + ">, " + fieldName + " ::" + fValue.toString());
		    			        	values.add(fValue);
		    			            types.add(field.getType());
		    			            ...
		    			            //Add to list
//		    			            Map<String, String> fieldMapping = m_vcMapper.getMapping(field.getType().getName());
//		    			            ParamListItem param = new ParamListItem(fieldMapping.get(VCMapper.ATTRIBUTENAME_DATATYPE), field.getClass().isPrimitive(), field.getName(), fValue, field.getClass().isArray());
//		    			            resultParamListItems.add(param);
		    					}
		    				}
    	      ...
    	      return new ParamListItem(getParamListFromFields( types.toArray(new Class[0]), labels.toArray(new String[0]), 
    		          values.toArray(new Object[0])), label, cl);	      	        	  
    		  ...
    		  //NOT ARRAY but registerde object
		      return new ParamListItem(getParamListFromFields( types.toArray(new Class[0]), labels.toArray(new String[0]), 
			          values.toArray(new Object[0])), label, cl);	      	        	  
...
//		          values.toArray(new Object[0])), label, cl);	      	        	  
  ...
  public ParamListItem getParamListItemFromFields22(
	      Class cl, String label, Object value) throws MdnException{
	    boolean isArray = cl.isArray();
...
*/	      
 ...
 //_________________________________________________________________________________
  ...
  public List<ParamListItem> getParamListFromConstructor(
      Class[] arguments, String[] labels) throws MdnException{
    List<ParamListItem> list = new ArrayList<ParamListItem>(arguments.length);
  ...
  public ParamListItem getParamListItemFromConstructor(
      Class cl, String label) throws MdnException{
    boolean isArray = cl.isArray();
    // set cl to its componenttype if cl is an array
    if (isArray){
      cl = cl.getComponentType();
...
//      return new ParamListItem(mapping.get(TypeMapper.ATTRIBUTENAME_DATATYPE), cl.isPrimitive(),
//          label+" ("+cl.getSimpleName()+")", (isArray)? new Object[]{value} : value, isArray);
      return new ParamListItem(mapping.get(TypeMapper.ATTRIBUTENAME_DATATYPE), cl.isPrimitive(),
              label , (isArray)? new Object[]{value} : value, isArray);      
    }
    // container item type
	    ...
	    for (int i = 0; i < args.length; i++){
	      ...
	      switch(paramListItem.getItemtype()){
	        case ParamListItem.ITEMTYPE_FIELD:
				...
				Object value = getInputValueList().get(0);//values.get(i);
  ...
  private Object setArgValue(String dataType, Object value){
	  ...
	  //When field type is Array or complex object so it will be pars as a list of simple type first
